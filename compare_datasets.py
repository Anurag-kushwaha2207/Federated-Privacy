...
print(f'Teeno mein common: {common_all}')
...
print(f'M1 & M2 same names: {len(m1m2)} -> {list(m1m2)[:3] if m1m2 else "KOI NAHI"}')
print(f'M1 & M3 same names: {len(m1m3)} -> {list(m1m3)[:3] if m1m3 else "KOI NAHI"}')
print(f'M2 & M3 same names: {len(m2m3)} -> {list(m2m3)[:3] if m2m3 else "KOI NAHI"}')
...
if len(m1m2) == 0 and len(m1m3) == 0 and len(m2m3) == 0:
    print('RESULT: TEENO DATASETS BILKUL ALAG HAIN!')
    print('  - Alag columns, alag domain, alag patients')
    print('  - Koi common data/patient nahi hai')
else:
    print(f'RESULT: Kuch overlap hai names mein!')
print('='*60)
